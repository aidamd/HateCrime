...
class MICNN():
    ...
    def build(self):
        ...
        for i, art_filter_size in enumerate(self.art_filter_sizes):
            ...
            conv = tf.nn.conv2d(state, art_W, strides=[1, 1, 1, 1], padding="SAME")
        ...
        self.drop = tf.reshape(self.sentence, [shape[0], -1 , 2 * self.hidden_size + art_num_filters_total])
    ...
    def run_model(self, batches, dev_batches, test_batches):
        ...
        with tf.Session() as self.sess:
            ...
            while True:
                ...
                for batch in batches:
                    feed_dict = {self.train_inputs: np.array([b[0] for b in batch]),
                                 ...
                                 self.article_lens: np.array([b[4] for b in batch])
                                 }
                    if self.pretrain:
                ...
                print(epoch, "Train accuracy:", train_accuracy / len(batches),
                      ...
                      "Hate F1:", f1_score(hate_true, hate_pred),
            ...
            if not self.test:
                for i in range(len(test_batches) // 5000 + 1):
                    print("Gathering labels for 5000 datapoints, batch #", i)
                    sub = test_batches[i * 5000: min((i + 1) * 5000, len(test_batches))]
